...
module String = struct
  ...
  let explode_array s = Array.init (String.length s) (fun i -> s.[i])
...
module Syntax = struct
  ...
  let ( let*| ) x y = match x with Some _ -> x | None -> y ()
  let ( let*& ) = Option.bind
  let ( let*% ) x y = match x with Some v -> v | None -> y ()
end
...
module InputUntil = struct
  ...
  let fold_substrings ?(input = stdin) n f acc =
    ...
    fold_lines ~input (fun acc s -> f acc (split_string s n)) acc
  ...
  let invalid_utf_seq () = invalid_arg "Invalid UTF-8 sequence"
  ...
  let fold_uchars ?(input = stdin) f acc =
    ...
    let get_uchar input =
      ...
      if Uchar.utf_decode_is_valid dec then Uchar.utf_decode_uchar dec
...
module Input = struct
  let fold_lines ?(input = stdin) f =
    InputUntil.fold_lines ~input (fun a e -> (true, f a e))
  ...
  let fold_fields ?(input = stdin) c f =
    InputUntil.fold_fields ~input c (fun a e -> (true, f a e))

  let fold_chars ?(input = stdin) f =
    InputUntil.fold_chars ~input (fun a e -> (true, f a e))

  let fold_uchars ?(input = stdin) f =
    InputUntil.fold_uchars ~input (fun a e -> (true, f a e))
end
...
module Ansi = struct
  ...
  let is_a_tty fmt =
    let open Format in
    let ofmt =
      if fmt == std_formatter then Some Unix.stdout
      else if fmt == err_formatter then Some Unix.stderr
      else None
    in
    match ofmt with None -> false | Some fd -> Unix.isatty fd
...
module Compare = struct
  ...
  let agg_list agg f l =
    ...
    | e :: l -> List.fold_left (fun acc e -> agg acc (f e)) (f e) l
  ...
  module Pqueue = struct
    include Set.Make (struct
      ...
      let compare (p1, v1) (p2, v2) =
        if p1 == p2 then compare v1 v2 else if p1 < p2 then -1 else 1
    end)
  ...
  let dijkstra ?(h = fun (_ : Graph.v) -> 0) ?(first = false) g start targets =
    ...
    let rec loop todo queue =
      if Pqueue.is_empty queue then finish_map
        ...
        if (finish_map %? u) && (prev %? u) then (
          let l = path_length global_dist prev u in
          finish_map.%{u} <- l;
          let todo = todo - 1 in
          if todo = 0 || first then finish_map else loop_aux todo nqueue u)
...
module Interval = struct
  ...
  let make_length a len = { inf = a; sup = a + len }
  let add t n = { inf = t.inf + n; sup = t.sup + n }
  let compare = compare

  let pp fmt t =
    let open Format in
    if t.inf = t.sup - 1 then fprintf fmt "%d" t.inf
    else fprintf fmt "[%d..%d)" t.inf t.sup

  let check t = if t.inf < t.sup then Some t else None
  let cap t1 t2 = check { inf = max t1.inf t2.inf; sup = min t1.sup t2.sup }

  let cup t1 t2 =
    let t1, t2 = if t2.inf > t1.inf then (t1, t2) else (t2, t1) in
    if t1.sup >= t2.inf then [ { t1 with sup = max t2.sup t1.sup } ]
    else [ t1; t2 ]
